...
function brs_check_ssid(cssid)
{
	...
	for(i=0;i<cssid.length;i++)
	{
		if(isValidChar_space(cssid.charCodeAt(i))==false)
		{
...
function validate_ad(cf){
	var ssid_ad = document.getElementById("ESSID_ad").value;
	if(!brs_check_ssid(ssid_ad))
		return false;
	for(i=0; i<cf.wig_ssid.value.length; i++)
	{
		var code = cf.wig_ssid.value.charCodeAt(i);
