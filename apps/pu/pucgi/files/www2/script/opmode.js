...
function getkey(type, e)
{
	...
	else if(type == "shareName")// not / * ? " <> \ : 
	{
        ...
        else if(type == "mediaServerName")// not / * ? " <> \ :
        {
			...
			alert("?z??????J  ':'?@? ReadyDLNA ?C?^?A????W?");  // || (keycode==58)
                        return false;
		}	
                else
                        return true;
...
function checkpsk(passphrase, passphrase_len)
{
	...
	if ( len == 64 )
	{
		for(i=0;i<len;i++)
		{
			...
			{
				alert("?K???8~63?ASCII?r???64??Q???????r????C?Q???????r??H?U???r????@?G0, 1, 2, ..., 8, 9, A, B, C, D, E, F?C");
				return false;
			}
		}
	}
...
function goTestApply()
{
	...
	if( run_test == "test")
	{
		if( in_hijack == true)// to fix bug 32600
			...
			window.open("http://www.netgear.com/success/r7500.aspx",null,winoptions);
...
function isValidDdns3322UserName(each_char)
{// 0-9, a-z
	if (!(((each_char>47) && (each_char<58))||((each_char>96) && (each_char<123)) || (each_char==8)||(each_char==0) || (each_char==95)))
		return false;
}
...
function wlan_txctrl(form, tx_power_ctrl, tx_power_ctrl_an, wla_channel, country)
{
	...
	if(pr_flag ==1 && super_wifi_flag == "1" && super_wifi_value_an != form.hid_super_wifi_an.value)
	{
		if( form.hid_super_wifi_an.value == "1")
			form.hid_super_wifi_reboot.value="1"; // from disable to enable
		else
			form.hid_super_wifi_reboot.value="2"; // from enable to disable
...
function select_type( type)
{
	...
	if(type == 0 )
		location.href="WLG_opmode_router_dual_band.htm";
	else if(type == 1 )
		location.href="WLG_opmode_ap_dual_band.htm";
	else if(type == 2 )
		location.href="WLG_opmode_bridge_dual_band.htm";
	else if(type == 3 )
		location.href="WLG_opmode_repeating_dual_band.htm";
...
function check_dev()
{
	...
	var cf = document.forms[0];
	var space_flag=0;
	var dev_name=cf.new_devname.value;
	warning=document.getElementById("devn_error");
	warning.style.display="none";

	if(dev_name == ""){
		warning.innerHTML="Device name can not be null.";
	...
	for(i=0;i<dev_name.length;i++){
		...
		{
			warning.innerHTML="Invalid device name.";
			warning.style.display="";
			return false;   
		}
	}
	if( dev_name.checkShareName()){
		warning.innerHTML="Invalid character or characters in "+"Network/Device Name";
		warning.style.display="";
		return false;
	}
	if(space_flag == 0){
	...
	if(typeof(ed_action)!="undefined")
		cf.action=ed_action;
	
	cf.submit_flag.value="edit_devname";
//	cf.submit();
	location.reload();
}
